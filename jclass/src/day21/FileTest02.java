...
public class FileTest02 {
	...
	public FileTest02() {
		...
		for(int i =0; i<flist.length; i++) {
			...
			String fileName = fileInfo.getName();//파일 이름 꺼내주는 함수
		...
		*/
		...
		String[] list = dir.list(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				 ...
				 * 
				 * 이 함수는 자동 호출함수이다.
				 * list() 함수가 호출될 때 자동으로 호출되는 함수이다.
				 * 목록을 구하는 파일을 하나씩 발견할 때마다 이 함수가
				 * 호출되는데 이때 반환값이 true이면 목록에 포함시키고
				 * 							false이면 목록에서 제외시킨다.
				 */
