...
public class Circle {
	...
	public void setRad(int rad) {
		this.rad = rad;//반지름이 셋팅되면 나머지 둘레와 면적도 구할수 있다
		setArround(); 	//반지름이 입력된 후 계산해서 둘 다 기억시켜놓기로 한다.
		setArea();
	}
	...
	public String toString() {
		return "반지름이"+rad+"이고 둘레가"+arround+"이고 면적은"+area+"인 원";
	}
	...
	public boolean equals(Object obj) {
		 ...
		 * 방법은 우리가 이 함수를 사용할 때는 circle1.equals(circle2)
		 * 이런 형태로 사용하는데 이때 각 객체의 멤버변수중 rad를 비교해서
		 * 그 값이 같으면 true를 반환해주고 다르면 false를 반환해주면 되겠다.
		 * 
		 */
		//1. 먼저 매개변수를 이 클래스 타입으로 강제 형변환 해준다.
				Circle tmp =(Circle) obj;
		//2. 현재 실행중인 객체의 멤버 rad 와 매개변수로 입력된 객체의 멤버 rad를 비교해보자
