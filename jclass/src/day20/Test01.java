...
public class Test01 {
	...
	public Test01() {
		...
		tab.put("이름", "홍길동");
		/*
		 * Map 계열은 데이터를 입력할 때 그 데이터를 구분하는 
		 * 키값과 데이터를 한쌍으로 만들어서 입력해야한다.
		 */
		tab.put("나이", 18);
		...
		System.out.println("이름 : "+ tab.get("이름"));
		/*
		 * 같은 키값으로 데이터를 입력하면 먼저 저장된 데이터는 
		 * 삭제되고 새로운 데이터가 입력된다.
		 * 즉, 같은 키값으로 한개의 데이터만 기억할 수 있다.
		 * 
		 */
		tab.put("성명","고길동");
		...
		while(itor.hasNext()) {
			...
			if(o instanceof String) {
				...
				int no =(int) o;
				System.out.println("정수열데이터: " + no);
			}else if(o instanceof Double) {
				Double num = (double) o; //Unboxing이 자동 처리됨
				System.out.println("실수데이터: "+ num);
			}
		}
		//키값만 추출..
		//키값을 추출하는 함수는  keys(), keySet()
		Enumeration en = tab.keys();
		//출력
		while(en.hasMoreElements()) {
			Object obj =en.nextElement();
			//우리의 경우 키값이 모두 문자열이기 때문에 강제 형변환 해주자
	...
	public void keyNval() {
		...
		//이 경우 Set데이터 안에 tab에 담겨 있던 키값과 데이터가 한 쌍으로 Map.Entry클래스에 저장되어있다.
		...
		while(itor.hasNext()) {
			 ...
			 * 따라서 ent안에 기억된 키값과 벨류(데이터)를 함수를 호출해서 꺼내면 된다.
			 */
	...
	public void keyNval2() {
		...
		//이때 list에는 Map.Entry타입의 데이터들이 들어있다.
